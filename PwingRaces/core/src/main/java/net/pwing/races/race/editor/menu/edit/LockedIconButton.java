...
public class LockedIconButton extends EditorButton {

    public LockedIconButton(RaceEditMenu menu, Race race) {
        super(menu, race);
    }

    @Override
    public ItemStack getItem() {
        ItemStack icon = this.race.getIconData().getLockedIcon().orElse(this.race.getIconData().getUnlockedIcon());
        List<String> lore = new ArrayList<>(Arrays.asList(
                "&7Click to change the race icon shown for players",
                "&7that have it locked.",
                "",
                "&fCurrent:",
        ...
        return ItemBuilder.builder(Material.REDSTONE_BLOCK)
                .name("&aRace Icon &c(Locked)")
                .lore(lore)
                .build();
    }
    ...
    public void onClick(Player player, ClickType action) {
        RaceEditorWizards.createWizard(BaseEditorContext::new)
                .addStage(new ItemStackInputStage<>(ChatColor.AQUA + "Select the item in your inventory to use as the locked race icon.", context -> this.race.getIconData()::setLockedIcon))
                .onComplete(context -> {
                    this.race.save();

                    context.getPlayer().sendMessage(ChatColor.GREEN + "Changed the locked race icon successfully!");
