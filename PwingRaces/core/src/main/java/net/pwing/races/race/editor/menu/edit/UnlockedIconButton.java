...
public class UnlockedIconButton extends EditorButton {

    public UnlockedIconButton(RaceEditMenu menu, Race race) {
        super(menu, race);
    }

    @Override
    public ItemStack getItem() {
        ItemStack icon = this.race.getIconData().getUnlockedIcon();
        List<String> lore = new ArrayList<>(Arrays.asList(
                "&7Click to change the race icon shown for players",
                "&7that have it unlocked.",
                "",
                "&fCurrent:",
        ...
        return ItemBuilder.builder(Material.EMERALD_BLOCK)
                .name("&aRace Icon (Unlocked)")
                .lore(lore)
                .build();
    }
    ...
    public void onClick(Player player, ClickType action) {
        RaceEditorWizards.createWizard(BaseEditorContext::new)
                .addStage(new ItemStackInputStage<>(ChatColor.AQUA + "Select the item in your inventory to use as the unlocked race icon.", context -> this.race.getIconData()::setUnlockedIcon))
                .onComplete(context -> {
                    this.race.save();

                    context.getPlayer().sendMessage(ChatColor.GREEN + "Changed unlocked race icon successfully!");
