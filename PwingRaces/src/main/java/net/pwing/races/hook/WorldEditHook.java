...
import net.pwing.races.hook.worldedit.IWorldEditHandler;
import net.pwing.races.hook.worldedit.WorldEditHandlerDisabled;
	...
	public void enableHook(PwingRaces owningPlugin, Plugin hook) {
		...
		try {
			if (hook.getDescription().getVersion().startsWith("7")) {
				clazz = Class.forName("net.pwing.races.hook.worldedit.WorldEditHandler_v7");
			} else if (hook.getDescription().getVersion().startsWith("6")) {
				clazz = Class.forName("net.pwing.races.hook.worldedit.WorldEditHandler_v6");
			}
