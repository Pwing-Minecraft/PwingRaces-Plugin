...
// TODO: Possible addition, ID support for older versions if people ask?
public enum RaceMaterial {
    ...
    RaceMaterial(int data, String... materials) {
    ...
    public boolean isDamageable(RaceMaterial type){
        if (type == null)
        	return false;
    ...
    public boolean isSameMaterial(ItemStack comp){
        ...
        if (comp.getType() == this.parseMaterial() && PwingRaces.getInstance().getCompatCodeHandler().getDamage(comp) == (int) this.data)
            return true;

        RaceMaterial mat = fromMaterial(comp.getType());
    ...
    private static HashMap<String, RaceMaterial> cachedSearch = new HashMap<String, RaceMaterial>();

    public static RaceMaterial fromMaterial(Material mat){
        try {
            return RaceMaterial.valueOf(mat.toString());
        } catch (IllegalArgumentException ex) {
            for (RaceMaterial raceMat : RaceMaterial.values()){
                for (String test : raceMat.materials) {
    ...
    public static RaceMaterial requestRaceMaterial(String name, int data){
        ...
        for (RaceMaterial mat : RaceMaterial.values()) {
